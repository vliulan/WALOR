...
//extern crate raft; // wyy add

use std::process::*;    // wyy add
...
//use tikv::raft::raft::test_raft; // wyy add
use tikv::raftstore::store::peer::is_ssd; // wyy add
...
fn overwrite_config_with_cmd_args(config: &mut TiKvConfig, matches: &ArgMatches) {
    // wyy add.
    if let Some(disk) = matches.value_of("disk") {
        if disk == "SSD" {
            //减小SSD的选举超时时间
            println!("SSD!");
	    unsafe {
	...
	// wyy add
        ...
        /*// test read
        let c1 = String::from("fio -filename=");
        let c2 = String::from("_fio -direct=1 -iodepth 1 -thread -rw=write -ioengine=psync -bs=4K -size=1G -numjobs=10 -runtime=30 -group_reporting -name=mytest");
        let c = c1 + &data_dir + &c2;
        println!("c: {:?}", c);
        ...
        let fio = String::from_utf8_lossy(&output.stdout);
        println!("fio: {}", fio);

        let mut speed_sum = 0;
        let mut num = 0;
        //let fio = String::from("io=134640KB, aggrb=4478KB/s, minb=4478KB/s, maxb=4478KB/s, mint=30064msec, maxt=30064msec WRITE: io=126440KB, aggrb=4205KB/s, minb=4205KB/s, maxb=4205KB/s, mint=30064msec, maxt=30064msec");
        let f: Vec<_> = fio.split(' ').collect();
        for element in f.iter() {
            let mut kb = 1;
            if(element.starts_with("aggrb")) {
                //println!("element: {:?}", element);
                if(element.contains("MB/s")) {
                    kb = 1024;
                }
                let speed: Vec<_> = element.split(|c| c == '=' || c == 'K' || c == '.' || c == 'M').collect();
                let s: i32 = speed[1].parse().unwrap();
                println!("speed[1]: {:?}",  s);
                speed_sum += s * kb;
                num += 1;
            }
        ...
        if(num != 0) {
            println!("speed_sum: {:?}, num: {:?}, speed: {:?}", speed_sum, num, speed_sum / num);
            labels.insert("LeaderWeight".to_owned(), (speed_sum / num).to_string().to_owned());
            //labels.insert("RegionWeight".to_owned(), (speed_sum / num).to_string().to_owned());
            //config.server.labels = labels;
        }*/

        // test write
        let c1 = String::from("fio -filename=");
        let c2 = String::from("_fio -direct=1 -iodepth 1 -thread -rw=randwrite -ioengine=psync -bs=8M -size=1G -numjobs=10 -runtime=30 -group_reporting -name=mytest");
        let c = c1 + &data_dir + &c2;
        println!("c: {:?}", c);
        let output = if cfg!(target_os = "windows") {
            Command::new("cmd")
