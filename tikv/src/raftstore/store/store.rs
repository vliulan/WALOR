...
impl<T: Transport, C: PdClient> Store<T, C> {
    ...
    fn on_raft_base_tick(&mut self, event_loop: &mut EventLoop<Self>) {
	    ...
	    //println!("on_raft_base_tick peer.raft_group.raft region_id: {:?}, term: {:?}, vote: {:?}, id: {:?}, state: {:?}", peer.region().id, peer.raft_group.raft.term, peer.raft_group.raft.vote, peer.raft_group.raft.id, peer.raft_group.raft.state);   // wyy add
