...
impl EngineRocksdb {
	...
	// wyy add
        println!("EngineRocksdb: creating for path {:?}", path); // wyy add
        let out = format!("EngineRocksdb: creating for path {:?}", path); // wyy add
        create_file("test_rocksdb.txt", &out.as_bytes());   // wyy add
